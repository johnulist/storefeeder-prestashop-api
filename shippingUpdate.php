...
try {
    ...
    $storefeederRef = mysql_real_escape_string($_GET['orderref']);
    $explodedRef = explode('-', $storefeederRef);
    $orderRef = $explodedRef[0];
    $date = date('Y-m-d H:i:s');
    $carrierName = mysql_real_escape_string($_GET['carriername']);
    $trackingNumber = mysql_real_escape_string($_GET['trackingnumber']);
    
    //run the inculde file to evaluate the $shippingUpdateQuery
    require 'query.inc.php';

    mysql_query($shippingUpdateQuery);
    mysql_query($orderHistoryQuery);
